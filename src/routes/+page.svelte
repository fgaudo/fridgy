<script lang="ts">
	...
	import { runViewModel } from './(viewmodel)/index.ts'
	import { Message } from './(viewmodel)/update.ts'

	const { executor } = getGlobalContext()

	const StateSchema = Schema.mutable(
		Schema.Struct({
			currentTimestamp: Integer.Schema,
			isMenuOpen: Schema.Boolean,
		}),
	)
	type State = Schema.Schema.Type<typeof StateSchema>

	const { state: viewmodelState, dispatch } = useViewmodel(
		executor,
		runViewModel,
	)

	const state = $state<State>(
		Schema.decodeSync(StateSchema)({
			isMenuOpen: false,
	...
	const maybeNonEmptySelected = $derived(
		Option.fromNullable(viewmodelState?.selectedProducts).pipe(
			Option.filter(s => HashSet.size(s) > 0),
		),
	)

	const refreshTimeListenersEnabled = $derived(
		viewmodelState?.products !== undefined &&
			viewmodelState.products.findIndex(
	...
	const hasSelectedProducts = $derived(
		Option.fromNullable(viewmodelState?.products).pipe(
			Option.map(products => Arr.length(products) > 0),
			Option.getOrElse(() => false),
		),
	)

	const maybeToastMessage = $derived(
		Option.fromNullable(viewmodelState?.toastMessage),
	)
</script>

{#if !viewmodelState || !dispatch}
	<div>Loading</div>
{:else}
	<div in:fade>
		{#if state.isMenuOpen}
		...
		<div class="fixed w-full z-50 top-0">
			...
			>
				...
				>
					{#if Option.isSome(maybeNonEmptySelected)}
				...
				{#if Option.isSome(maybeNonEmptySelected)}
					...
					>
						{#if !viewmodelState.isDeleteRunning}
							<Ripple
								ontap={() => {
									dispatch(Message.StartDeleteSelectedAndRefresh())
								}}
							></Ripple>{/if}
						<Trash2 />
					</div>
			...
			{#if Option.isSome(maybeLoadedNonEmptyProducts)}
				{@const products = maybeLoadedNonEmptyProducts.value}
		...
		<div class="bg-background flex flex-col">
			...
			{:else if Option.isSome(maybeLoadedNonEmptyProducts)}
				{@const products = maybeLoadedNonEmptyProducts.value}
								...
								>
									...
									>
										...
										{#if Option.isNone(maybeCreation)}
											No creation date
										{:else if Option.isSome(maybeExpirationDate) && maybeExpirationDate.value > viewmodelState.currentTimestamp}
											{@const expiration = maybeExpirationDate.value}
											{@const creation = maybeCreation.value}

											{@const totalDuration = expiration - creation}
											{@const remainingDuration =
												expiration - viewmodelState.currentTimestamp}
												...
												class={[
													`text-primary duration-fade absolute text-sm`,
													{
														'text-primary font-bold':
															...
															viewmodelState.currentTimestamp,
													},
												]}
											>
		...
		{#if Option.isNone(maybeNonEmptySelected)}
			<div
				class="fixed right-[16px] left-[16px]"
				style:bottom={`calc(var(--safe-area-inset-bottom, 0) + 21px)`}
			>
				{#if !viewmodelState.receivedError && Option.isNone(maybeLoadedNonEmptyProducts)}
					<div
		...
		{#key Option.isSome(maybeToastMessage)}
			<div
				in:fade={{
					duration: 300,
					easing: cubicIn,
				...
				style:bottom={Option.isNone(maybeNonEmptySelected)
			...
			>
				{#if Option.isSome(maybeToastMessage)}
					<div
