<script lang="ts">
	...
	const { state: viewModelState } = $derived(
		useViewmodel({
			...
			messages: m => {
				...
				} else if (m._tag === 'DeleteSucceededButRefreshFailed') {
	...
	$effect(() => {
		if (viewModelState.canFetch._tag === 'False') {
			return
		}

		const fetch = viewModelState.canFetch.fetch
		const cancel = runtime.runCallback(
			Stream.runForEach(createCapacitorListener('resume'), () => fetch),
		)
		return cancel
	})
...
<div in:fade>
	...
	<div class="fixed w-full z-50 top-0">
		...
		>
			...
			>
				{#if viewModelState.productListStatus._tag === 'Available' && viewModelState.productListStatus.hasSelectedProducts._tag === 'True'}
					<div class="absolute" transition:fade={{ duration: 200 }}>
						{#if viewModelState.productListStatus.canClearSelection._tag === 'True'}
							{@const clear =
								viewModelState.productListStatus.canClearSelection.clear}
							<Ripple
								ontap={() => {
									runtime.runCallback(clear)
								}}
							></Ripple>
						{/if}
			...
			{#if viewModelState.productListStatus._tag === 'Available' && viewModelState.productListStatus.hasSelectedProducts._tag === 'True'}
				...
				>
					{#key viewModelState.productListStatus.hasSelectedProducts.number}
						<div class="absolute" transition:fade={{ duration: 200 }}>
							{viewModelState.productListStatus.hasSelectedProducts.number}
						</div>
					{/key}
				</div>
				...
				<div
					...
					transition:fade={{ duration: 200 }}
				>
					{#if viewModelState.productListStatus.canDelete._tag === 'True'}
						{@const deleteSelected =
							viewModelState.productListStatus.canDelete.deleteSelected}
						<Ripple
							ontap={() => {
								runtime.runCallback(deleteSelected)
							}}
						></Ripple>
					{/if}
					<Trash2 />
	...
	<div class="bg-background flex flex-col">
		{#if viewModelState.productListStatus._tag === 'Error'}
			{#if viewModelState.isFetching}
				<div
					transition:fade={{ duration: 200 }}
			...
			{:else if viewModelState.canFetch._tag === 'True'}
				{@const fetch = viewModelState.canFetch.fetch}
				<div
			...
			{/if}
		{:else if viewModelState.productListStatus._tag === 'Initial'}
			<div
				class="flex h-screen w-screen items-center justify-center text-center text-lg"
			>
				<Spinner />
			</div>
		{:else if viewModelState.productListStatus._tag === 'Available'}
			{@const products = viewModelState.productListStatus.products}

			{#if viewModelState.productListStatus.isDeleting}
				<div
					transition:fade={{ duration: 200 }}
					class="z-50 scale-[175%] fixed left-0 top-0 right-0 bottom-0 backdrop-blur-[1px] flex items-center justify-center"
				>
				...
				{#each products as product (product.id)}
					<div out:fade={{ duration: 200 }} animate:flip={{ duration: 250 }}>
						...
						{:else if product._tag === 'Invalid'}
							...
							>
								{#if product.canToggle._tag === 'True'}
									{@const toggle = product.canToggle.toggle}
									{#if viewModelState.productListStatus.hasSelectedProducts._tag === 'True'}
										<Ripple
											ontap={() => {
												runtime.runCallback(toggle)
											}}
										></Ripple>
									{:else}
										<Ripple
								...
								>
									...
									>
										...
										{:else if product.status._tag === 'Stale'}
											<div class="text-[12px] leading-2 text-primary font-bold">
												Expired
											</div>
										{/if}
									</div>
									...
									>
										{#if product.status._tag === 'Fresh'}
											<div
												class={[`text-primary duration-fade absolute text-sm`]}
											>
		...
		{#if (viewModelState.productListStatus._tag !== 'Initial' && viewModelState.productListStatus._tag !== 'Available') || (viewModelState.productListStatus._tag === 'Available' && viewModelState.productListStatus.hasSelectedProducts._tag === 'False')}
			<div
				class="fixed right-4 left-4"
				style:bottom={`calc(var(--safe-area-inset-bottom, 0) + 21px)`}
			>
