import { A, Eff, Ref, flow } from './imports.ts'
...
export function createDispatcher<S, M, R>(
	ref: Ref.Ref<S>,
	update: Update<S, M, R>,
): (command: Command<M, R>) => Eff.Effect<void, never, R> {
	return command =>
		Eff.gen(function* () {
			const message = yield* command

			const { commands } = yield* Ref.modify(ref, state => {
				const { state: newState, commands } = update(state, message)
			...
			if (commands) {
				const dispatchesPerTask = A.map(
					...
					flow(
						command =>
							Eff.suspend(() => createDispatcher(ref, update)(command)),
						Eff.forkDaemon,
					),
				)
