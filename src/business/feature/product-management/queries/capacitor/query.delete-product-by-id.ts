...
import * as DeleteProductById from '../delete-product-by-id'
import { Db } from './db'

export const command = Layer.effect(
	DeleteProductById.DeleteProductById,
	Effect.gen(function* () {
		const { deleteProductsByIds } = yield* Db

		return {
			resolver: RequestResolver.makeBatched<DeleteProductById.Request, never>(
				Effect.fn(
					function* (requests) {
