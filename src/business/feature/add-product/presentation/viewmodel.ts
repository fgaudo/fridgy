...
import * as AddProduct from '@/feature/add-product/app/usecase/add-product.ts'
import * as UseCases from '@/feature/add-product/app/usecase/index.ts'
...
type InternalMessage = Message | AddProduct.Message
...
const matcher = Match.typeTags<
	...
	(s: Readonly<State>) => Readonly<{
		state: State
		commands: Command<InternalMessage, UseCases.UseCases>[]
	}>
>()
	...
	AddProduct:
		...
		state => {
			...
			return modify(state, draft => {
				...
				return [AddProduct.AddProduct.run({ name, maybeExpirationDate })]
			})
		},
	AddProductFailed: () =>
		modify(draft => {
