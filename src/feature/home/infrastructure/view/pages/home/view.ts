...
import type { Message } from '@/app/core/home/messages.ts'
import type { Model } from '@/app/core/home/model.ts'
import { cn } from '@/shared/helpers1.ts'

type HomeDeps = { dispatch: (m: Message) => void }

function lockScroll(isLocked: boolean) {
  document.body.style.overflow = isLocked ? 'hidden' : ''
}

export const makeView = ({ dispatch }: HomeDeps) => (model: Model, route: 'add' | 'default') => {
    ...
    [
      h(
        'div',
        {
          ...
          on: { click: () => dispatch({ _tag: 'AddProductClosed' }) },
          props: {
